...
export class PurchaseTicketModel {
  ...
  public constructor(data: Partial<PurchaseTicketModel> = {}) {
    this.fill(data);
  }

  private fill(data: Partial<PurchaseTicketModel>) {
    this.id = data.id ?? 0;
  ...
  public static async findAll(): Promise<PurchaseTicketModel[]> {
    const db = Database.getInstance();
    const [rows] = await db.execute<RowDataPacket[]>(
